    ...
    return new Promise((resolve, reject) => {
      ...
      newUser
        ...
        .catch(err => {
          ...
          else resolve({ status: false, message: "already_exist" });
        });
    });
  }
  ...
  update(iduser, params) {
    return new Promise((resolve, reject) => {
      if (params["isAdmin"])
        resolve({
          status: false,
          message: "Are you serious ? Go fuck another guy bro"
        });
      if (params["id"])
        resolve({
          status: false,
          message: "Don't try to do shit with my application blyat"
        });
      this.db.User.update(params, { where: { id: iduser } })
        .then(rowUpdated => {
